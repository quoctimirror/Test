...
export function calculateRingTarget(handLandmarks) {
    ...
    if (!handLandmarks || !handLandmarks[9] || !handLandmarks[13] || !handLandmarks[14]) {
        return null;
    }
    ...
    const upDir = new THREE.Vector3().crossVectors(forwardDir, sidewaysDir).normalize();

    // Tạo ma trận xoay từ 3 vector hướng này
    const rotationMatrix = new THREE.Matrix4().makeBasis(sidewaysDir, upDir, forwardDir);
