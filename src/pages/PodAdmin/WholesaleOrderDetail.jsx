...
export default function AdminWholesaleOrderDetail() {
  ...
  const fetchOrder = async () => {
    try {
      setLoading(true);
      const response = await adminWholesaleApi.getOrder(orderId);
  ...
  const handleApprove = () => {
    setModal({
      title: "Approve Order",
      message: "Are you sure you want to approve this wholesale order?",
      confirmText: "Approve",
      confirmStyle: "pod-btn pod-btn-success",
      onConfirm: async () => {
        setModal(null);
        try {
          await adminWholesaleApi.approve(orderId);
          fetchOrder();
        } catch (err) {
          showError(err);
        }
  ...
  const handleProcess = () => {
    setModal({
      title: "Process Order",
      message: "Start processing this order? This means items are being prepared for shipment.",
      confirmText: "Start Processing",
      confirmStyle: "pod-btn pod-btn-primary",
      onConfirm: async () => {
        setModal(null);
        try {
          await adminWholesaleApi.process(orderId);
          fetchOrder();
        } catch (err) {
          showError(err);
        }
  ...
  const handleShip = () => {
    ...
    setModal({
      title: "Ship Order",
      message: "Enter the tracking number for this shipment.",
      inputLabel: "Tracking Number",
      inputPlaceholder: "e.g. GHN123456789",
      confirmText: "Ship Order",
      confirmStyle: "pod-btn pod-btn-primary",
      onConfirm: async (inputValue) => {
        setModal(null);
        try {
          await adminWholesaleApi.ship(orderId, inputValue?.trim() || "");
          fetchOrder();
        } catch (err) {
          showError(err);
        }
  ...
  const handleComplete = () => {
    setModal({
      ...
      message: "Mark this order as completed? This is the final step.",
      confirmText: "Complete",
      confirmStyle: "pod-btn pod-btn-success",
      onConfirm: async () => {
        setModal(null);
        try {
          await adminWholesaleApi.complete(orderId);
          fetchOrder();
        } catch (err) {
          showError(err);
        }
  ...
  const handleCancel = () => {
    ...
    setModal({
      ...
      onConfirm: async (inputValue) => {
        ...
        setModal(null);
        try {
          await adminWholesaleApi.cancel(orderId, inputValue.trim());
          fetchOrder();
        } catch (err) {
          showError(err);
        }
    ...
    <div className="pod-page">
      <div className="pod-page-header">
        <div>
          <button
            ...
            onClick={() => navigate(ROUTES.POD_ADMIN_WHOLESALE_ORDERS)}
            style={{ marginBottom: "0.5rem" }}
          >
            &larr; Back to Orders
          </button>
          <h1 className="pod-page-title">Wholesale Order (Admin)</h1>
        </div>
        <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
          {order?.status === "SUBMITTED" && (
            <button className="pod-btn pod-btn-success" onClick={handleApprove}>Approve</button>
          )}
          {order?.status === "APPROVED" && (
            <button className="pod-btn pod-btn-primary" onClick={handleProcess}>Process</button>
          )}
          {order?.status === "PROCESSING" && (
            <button className="pod-btn pod-btn-primary" onClick={handleShip}>Ship</button>
          )}
      ...
      <div className="pod-card" style={{ marginBottom: "1.5rem" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1.5rem" }}>
          <div>
            <div style={{ fontSize: "0.875rem", color: "#6b7280" }}>Partner</div>
            <div style={{ fontWeight: "500" }}>{order?.partnerName || order?.partnerId}</div>
          </div>
          <div>
          ...
          <div>
            ...
            <div style={{ fontWeight: "700", fontSize: "1.25rem" }}>{formatCurrency(order?.totalAmount)}</div>
          </div>
