    ...
    try {
      if (editingComponent) {
        ...
        if (response.status === 200) {
            ...
            components.map((comp) =>
              comp.id === editingComponent.id
                ? {
                    ...
                    categoryName:
                      updatedComponent.category?.categoryName ||
                      "Unknown Category",
                  }
                : comp
            )
          ...
          setErrorMessage(
            response.data?.message || "Failed to update component"
          );
          return;
        }
      } else {
        ...
        if (response.status === 201 || response.status === 200) {
          ...
          setComponents([
            ...components,
            {
              ...
              categoryName:
                newComponent.category?.categoryName || "Unknown Category",
            },
          ]);
        } else {
          console.error("Failed to create component:", response.data);
          setErrorMessage(
            response.data?.message || "Failed to create component"
          );
          return;
        }
      }
